...
int scan(Mat *matrix, struct node *n0){
	...
	cvtColor(*matrix,bwMat,CV_BGR2GRAY,0);
	threshold(bwMat,bwMat,50,255,THRESH_BINARY);

	//imshow("bw",bwMat);

//	rectangle( *matrix,Point(x0,y0),Point(x0+40,y0), const Scalar& color, int thickness=1, int lineType=8, int shift=0 )

	for(int i=x0; i<(x0+80); i++){
		for(int j=y0; j<(y0+80); j++){
//			printf("%d ",(int)bwMat.at<unsigned char>(j,i));
			if( (int)bwMat.at<unsigned char>(j,i)  == 0 )
				return 1;
		}
//	printf("\n");
	}
...
int loadMap(Mat* matrix,struct node *n0, int nodeCount){
	...
	for(int i=0; i<nodeCount; i++){
		...
		/*if(n0[i].pl > 0)
		circle( tempMat, Point(x,y), (int)*(&n0[i].pl), Scalar(255,255,255), -1, 8, 0 );

		else
		circle( tempMat, Point(x,y), 40, Scalar(30,30,30), -1, 8, 0 );
		*/
		

		if( (int)*(&n0[i].type) == 1 )
			circle( tempMat, Point(x,y), 5 , Scalar(0,255,0), -1, 8, 0 );
...
void updateFrontiers(Mat *matrix,Point *frontiers,int frontierCount){
	...
	for(int i=0; i<frontierCount; i++)
		circle( frontierMap, frontiers[i], 2, Scalar(0,0,255), -1, 8, 0 );
