 ...
 */
int loadMap(Mat* matrix,Mat* bw,struct node *n0, int nodeCount ){
	...
	cvtColor(*matrix,*bw,CV_BGR2GRAY,0);
...
int findUtility(Mat *mat, int x0, int y0, int x, int y){

	int utility=0;

	// iterate along the path from src to dest
	double theta = calcTheta(x0,y0,x,y);
	double dist =  calcDist (x0,y0,x,y);

	int size = dist/20;

	// sine, cosine values
	double sv = sin( theta * 22 / (7 * 180) );
	double cv = cos( theta * 22 / (7 * 180) );

	//printf("\nTheta : %.4f",theta);
	//printf("\n\nSin Val : %.4f , Cos Val : %.4f",sv,cv);

	for(int d=0; calcDist( x,y, (double)(d*cv), (double)(d*sv) ) ; d+=5)
		utility += (*mat).at<unsigned char>( (int)(d*cv), (int)(d*sv) ) / 255;
		

//	printf("Util @ (%d,%d) : %d",x0,y0,utility);
	return utility;
...
void assignBestCell(struct node *n0, Mat *bw){

	double max = -100000;
	int x,y;


	for(int i=0; i<gMax_X; i+=25){
		for(int j=0; i<gMax_Y; j+=25){
		
			int val = findUtility(bw,n0->x,n0->y,i,j) - calcDist(n0->x,n0->y,i,j);
